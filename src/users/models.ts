 ...
 */
export async function createUserFromFirebaseAuth(firebaseUser: UserRecord) {
  ...
  const collectionExists = (await firebase.firestore().collection('users').get()).size !== 0;
  ...
  return new User({
    ...
    role: collectionExists ? 'guest' : 'owner'
  } as User);
}